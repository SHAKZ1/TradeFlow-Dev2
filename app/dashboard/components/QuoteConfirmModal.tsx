...
export function QuoteConfirmModal({ isOpen, onClose, onConfirm, onSkip, clientName }: QuoteConfirmModalProps) {
    ...
    <div className="fixed inset-0 bg-gray-900/20 backdrop-blur-sm z-[10000] flex items-center justify-center p-4">
        ...
        >
            <div className="p-8 text-center">
                ...
                {/* TEXT */}
                <h3 className="text-xl font-bold text-gray-900 mb-3 tracking-tight">Generate Quote</h3>
                <p className="text-sm text-gray-500 leading-relaxed px-2">
                    <span className="font-semibold text-gray-900">{clientName}</span> is now in the quoting stage. Would you like to draft a deposit request?
                </p>
            </div>
            ...
            <div className="p-6 pt-0 flex flex-col gap-3">
                <button 
                    onClick={onConfirm}
                    className="w-full py-3.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-2xl shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2 transform hover:-translate-y-0.5 border-none outline-none cursor-pointer"
                >
